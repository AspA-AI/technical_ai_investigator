...
from agents.nodes.nodes import root_cause_analyzer_node, summary_generator_node
...
def test_summary_generator_node_uses_llm(monkeypatch) -> None:
    class FakeLLMClient:
        def generate_text(
            ...
            return (
                ...
                '"root_causes":[{"cause":"cooling degradation","confidence":88,"evidence":["Matched incident data"]}],'
                '"recommendations":["Inspect cooling system"],"action_plan":["Inspect cooling system"],'
                '"summary_text":"Generated investigation summary."}'
            )
    ...
    assert result["summary_sections"]["headline"] == "Investigation Summary"
...
def test_summary_generator_node_falls_back_when_llm_missing(monkeypatch) -> None:
    ...
    assert "bearing wear" in result["summary"]
    assert result["summary_sections"]["summary_text"].startswith("Detected anomalies")
