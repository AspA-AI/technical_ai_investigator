...
class ReportService:
    ...
    def _build_pdf(
        ...
        styles = {
            "title": ParagraphStyle(
                "Title", fontSize=20, leading=24, spaceAfter=14, alignment=1
            ),
            "heading": ParagraphStyle(
                "Heading", fontSize=14, leading=18, spaceAfter=10, spaceBefore=12
            ),
        ...
        story.append(
            Paragraph(f"Investigation ID: {investigation.id}", styles["normal"])
        )
        story.append(
            Paragraph(f"Upload ID: {investigation.upload_id}", styles["normal"])
        )
        story.append(Paragraph(f"Status: {investigation.status}", styles["normal"]))
        story.append(Spacer(1, 0.2 * inch))

        summary = get_summary_text(state) or "Summary not available."
        ...
        if recommendations:
            ...
            story.append(
                Paragraph("No formal recommendations were generated.", styles["normal"])
            )
    ...
    def _build_pdf_findings(
        self, state: dict[str, Any], styles: dict[str, ParagraphStyle]
    ) -> list[Paragraph]:
        ...
        if anomalies:
            ...
            findings.append(
                Paragraph("No anomaly patterns detected.", styles["normal"])
            )

        incidents = state.get("incidents") or []
        if incidents:
            findings.append(
                Paragraph("Relevant historical incidents:", styles["normal"])
            )
            ...
            findings.append(
                Paragraph(
                    "No relevant historical incidents were matched.", styles["normal"]
                )
            )
        ...
        if root_causes:
            ...
            for cause in root_causes:
                if isinstance(cause, dict):
                    findings.append(
                        Paragraph(
                            f"{cause.get('cause')} ({cause.get('confidence', 0)}% confidence)",
                            styles["bullet"],
                        )
                    )
        else:
            findings.append(
                Paragraph("No root cause hypotheses were identified.", styles["normal"])
            )
    ...
    def _build_presentation(
        ...
        slide.placeholders[
            1
        ].text = f"Investigation {investigation.id} • Upload {investigation.upload_id}"
        self._style_text_frame(slide.shapes.title.text_frame, 28, bold=True)
        self._style_text_frame(slide.placeholders[1].text_frame, 14)

        self._add_bullet_slide(
            presentation,
            "Executive Summary",
            [get_summary_text(state) or "Summary not available."],
        )
        ...
        if anomalies:
            ...
            for anomaly in anomalies:
                if isinstance(anomaly, dict):
                    findings.append(
                        ", ".join(
                            f"{k.replace('_', ' ')}" for k, v in anomaly.items() if v
                        )
                    )
        else:
        ...
        if incidents:
            ...
            for incident in incidents:
                if isinstance(incident, dict):
                    findings.append(
                        f"Incident {incident.get('incident_id')} (similarity {incident.get('similarity')})"
                    )
        ...
        if recommendations:
            self._add_bullet_slide(
                presentation, "Recommendations", [str(item) for item in recommendations]
            )
        else:
            self._add_bullet_slide(
                presentation, "Recommendations", ["No recommendations were generated."]
            )
    ...
    def _add_bullet_slide(
        self, presentation: Presentation, title: str, lines: list[str]
    ) -> None:
        slide_layout = presentation.slide_layouts[1]
        slide = presentation.slides.add_slide(slide_layout)
        slide.shapes.title.text = title
        self._style_text_frame(slide.shapes.title.text_frame, 24, bold=True)
        ...
        self._style_text_frame(body, 13)
        for line in lines[1:]:
    ...
    @staticmethod
    def _style_text_frame(text_frame, size: int, bold: bool = False) -> None:
